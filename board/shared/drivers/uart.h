 ...
 */
typedef void (*uart_callback_t)(const struct device *dev, struct uart_event *evt,
								void *user_data);
 ...
 * @retval -ENOTSUP If API is not enabled.
 */
static inline int uart_fifo_fill(const struct device *dev, const uint8_t *tx_data,
								 int size);
 ...
 * @retval -ENOTSUP If API is not enabled.
 */
static inline int uart_fifo_fill_u16(const struct device *dev, const uint16_t *tx_data,
									 int size);
 ...
 * @retval -ENOTSUP If API is not enabled.
 */
static inline int uart_fifo_read_u16(const struct device *dev, uint16_t *rx_data,
									 const int size);
 ...
 * @retval -errno Other negative errno value in case of failure.
 */
static inline int uart_tx_u16(const struct device *dev, const uint16_t *buf, size_t len,
							  int32_t timeout);
 ...
 *
 */
static inline int uart_rx_enable_u16(const struct device *dev, uint16_t *buf, size_t len,
									 int32_t timeout);
 ...
 * @retval -errno Other negative errno value in case of failure.
 */
static inline int uart_line_ctrl_set(const struct device *dev, uint32_t ctrl,
									 uint32_t val);
...
DEVICE_CLASS_DEFINE(uart_device,
	...
	int (*tx_u16)(const struct device *dev, const uint16_t *buf, size_t len,
				  int32_t timeout);
	int (*rx_enable_u16)(const struct device *dev, uint16_t *buf, size_t len,
						 int32_t timeout);
