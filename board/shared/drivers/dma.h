...
DEVICE_CLASS_DEFINE(dma_device,
...
static inline int dma_suspend(const struct device *dev, uint32_t channel)
{
	const struct dma_device *api = (const struct dma_device *)dev;
 ...
 * @retval Negative errno code if failure.
 */
static inline int dma_request_channel(const struct device *dev,
				  void *filter_param);
					     ...
					     void *filter_param)
{
	...
	struct dma_context *dma_ctx = (struct dma_context *)dev_get_private((struct device *)dev);
 ...
 *
 */
static inline void dma_release_channel(const struct device *dev,
				   uint32_t channel);
 ...
 *
 */
static inline int dma_chan_filter(const struct device *dev,
				   int channel, void *filter_param);
