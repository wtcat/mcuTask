		...
		if (urb_state != URB_NOTREADY) {
			...
			case URB_DONE:
				...
				if (ed->ux_stm32_ed_dir == 1) {
					if ((ed->ux_stm32_ed_type == EP_TYPE_CTRL) ||
						(ed->ux_stm32_ed_type == EP_TYPE_BULK)) {
						...
						if ((transfer_request->ux_transfer_request_requested_length >
							...
							(HAL_HCD_HC_GetXferCount(hcd_stm32->hcd_handle,
													 ed->ux_stm32_ed_channel) ==
							 ed->ux_stm32_ed_endpoint->ux_endpoint_descriptor
								 .wMaxPacketSize)) {
							/* Adjust the transmit length.  */
							ed->ux_stm32_ed_packet_length = UX_MIN(
								ed->ux_stm32_ed_endpoint->ux_endpoint_descriptor
									.wMaxPacketSize,
							...
							HAL_HCD_HC_SubmitRequest(
								hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 1U,
				...
				if (ed->ux_stm32_ed_dir == 0U) {
					...
					if ((hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].do_ssplit ==
						 1U) &&
						(ed->ux_stm32_ed_type == EP_TYPE_ISOC) &&
						(ed->ux_stm32_ed_packet_length >
						 hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].max_packet)) {
						/* Update actual transfer length with ISOC max packet size for
						 * split transaction  */
						transfer_request->ux_transfer_request_actual_length +=
							hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].max_packet;
					} else
					...
					if (transfer_request->ux_transfer_request_requested_length >
						transfer_request->ux_transfer_request_actual_length) {
#if defined(USBH_HAL_HUB_SPLIT_SUPPORTED)
						if ((hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel]
								 .do_ssplit == 1U) &&
							(ed->ux_stm32_ed_type == EP_TYPE_ISOC) &&
							(ed->ux_stm32_ed_packet_length >
							 hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel]
								 .max_packet)) {
							/* Adjust the transmit length.  */
							ed->ux_stm32_ed_packet_length =
							...
							/* Submit the transmit request.  */
							HAL_HCD_HC_SubmitRequest(
								hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 0,
								ed->ux_stm32_ed_type, USBH_PID_DATA,
								ed->ux_stm32_ed_data +
									transfer_request->ux_transfer_request_actual_length,
								ed->ux_stm32_ed_packet_length, 0);
			...
			if (transfer_next) {
				...
				if (!ed->ux_stm32_ed_sch_mode) {
					...
					if ((ed->ux_stm32_ed_type == EP_TYPE_ISOC) &&
						...
						ed->ux_stm32_ed_packet_length =
							transfer_next->ux_transfer_request_packet_length;
					else
						ed->ux_stm32_ed_packet_length =
							transfer_next->ux_transfer_request_requested_length;
					...
					HAL_HCD_HC_SubmitRequest(
						hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel,
						ed->ux_stm32_ed_dir, ed->ux_stm32_ed_type, USBH_PID_DATA,
						ed->ux_stm32_ed_data +
							transfer_next->ux_transfer_request_actual_length,
						ed->ux_stm32_ed_packet_length, 0);
				}
			} else {
			...
			if ((ed->ux_stm32_ed_status == UX_HCD_STM32_ED_STATUS_CONTROL_SETUP) ||
				...
				HAL_HCD_HC_SubmitRequest(
					hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 0,
					((ed->ux_stm32_ed_endpoint->ux_endpoint_descriptor.bmAttributes) &
...
static inline VOID _ux_hcd_stm32_request_control_setup(UX_HCD_STM32 *hcd_stm32,
	...
	setup_request =
		_ux_utility_memory_allocate(UX_NO_ALIGN, UX_CACHE_SAFE_MEMORY, UX_SETUP_SIZE);
	...
	*(setup_request + UX_SETUP_REQUEST) = transfer_request->ux_transfer_request_function;
	_ux_utility_short_put(setup_request + UX_SETUP_VALUE,
						  transfer_request->ux_transfer_request_value);
	_ux_utility_short_put(setup_request + UX_SETUP_INDEX,
						  transfer_request->ux_transfer_request_index);
	...
	ed->ux_stm32_ed_dir = 0;
	HAL_HCD_HC_Init(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 0,
					endpoint->ux_endpoint_device->ux_device_address,
					ed->ux_stm32_ed_speed, EP_TYPE_CTRL,
	...
	HAL_HCD_HC_SubmitRequest(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 0,
							 EP_TYPE_CTRL, USBH_PID_SETUP, setup_request, 8, 0);
}
...
static inline VOID _ux_hcd_stm32_request_control_data(UX_HCD_STM32 *hcd_stm32,
	...
	if ((transfer_request->ux_transfer_request_type & UX_REQUEST_DIRECTION) ==
		...
		HAL_HCD_HC_Init(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, 0x80,
						endpoint->ux_endpoint_device->ux_device_address,
						ed->ux_stm32_ed_speed, EP_TYPE_CTRL,
						endpoint->ux_endpoint_descriptor.wMaxPacketSize);
	...
	/* If the direction is OUT, request size is larger than MPS, and DMA is not used, we
	 * need to set transfer length to MPS.  */
	...
	HAL_HCD_HC_SubmitRequest(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel,
							 ed->ux_stm32_ed_dir, EP_TYPE_CTRL, USBH_PID_DATA,
							 ed->ux_stm32_ed_data, ed->ux_stm32_ed_packet_length, 0);
}
...
static inline VOID _ux_hcd_stm32_request_control_status(UX_HCD_STM32 *hcd_stm32,
	...
	HAL_HCD_HC_Init(
		hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, ed->ux_stm32_ed_dir ? 0x80 : 0,
		endpoint->ux_endpoint_device->ux_device_address, ed->ux_stm32_ed_speed,
		EP_TYPE_CTRL, endpoint->ux_endpoint_descriptor.wMaxPacketSize);
	...
	ed->ux_stm32_ed_saved_actual_length =
		(USHORT)transfer_request->ux_transfer_request_actual_length;
	transfer_request->ux_transfer_request_actual_length = 0;
...
static UX_HCD_STM32_ED *_ux_hcd_stm32_ed_obtain(UX_HCD_STM32 *hcd_stm32) {
	UX_HCD_STM32_ED *ed;
	ULONG ed_index;
...
STATIC_UINT _ux_hcd_stm32_endpoint_create(UX_HCD_STM32 *hcd_stm32, UX_ENDPOINT *endpoint) {
	UX_HCD_STM32_ED *ed;
	UX_DEVICE *device;
	ULONG channel_index;
	UINT device_speed;
	...
	if ((endpoint_type == EP_TYPE_INTR) || (endpoint_type == EP_TYPE_ISOC)) {
		if (device_speed == HCD_DEVICE_SPEED_HIGH) {
			if ((device->ux_device_current_configuration->ux_configuration_first_interface
					 ->ux_interface_descriptor.bInterfaceClass == 0x9U) &&
		...
		if (ed->ux_stm32_ed_interval_mask == 0)
			...
			ed->ux_stm32_ed_interval_position =
				_ux_hcd_stm32_least_traffic_list_get(hcd_stm32);
	...
	ed->ux_stm32_ed_dir =
		(endpoint->ux_endpoint_descriptor.bEndpointAddress & 0x80) ? 1 : 0;
	ed->ux_stm32_ed_type = endpoint_type;
	packet_size =
		endpoint->ux_endpoint_descriptor.wMaxPacketSize & UX_MAX_PACKET_SIZE_MASK;
	if (endpoint->ux_endpoint_descriptor.wMaxPacketSize &
	...
	HAL_HCD_HC_Init(hcd_stm32->hcd_handle, channel_index,
					endpoint->ux_endpoint_descriptor.bEndpointAddress,
					device->ux_device_address, device_speed, endpoint_type,
					endpoint->ux_endpoint_descriptor.wMaxPacketSize);
	...
	endpoint->ux_endpoint_transfer_request.ux_transfer_request_maximum_length =
		UX_HCD_STM32_MAX_PACKET_COUNT * packet_size;

	/* Return successful completion.  */
	return (UX_SUCCESS);
}

STATIC_UINT _ux_hcd_stm32_endpoint_destroy(UX_HCD_STM32 *hcd_stm32, UX_ENDPOINT *endpoint) {
#if defined(UX_HOST_STANDALONE)
	...
	if (ed == UX_NULL) {
		/* Error trap. */
		_ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_HCD,
								 UX_ENDPOINT_HANDLE_UNKNOWN);

		/* If trace is enabled, insert this event into the trace buffer.  */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_ENDPOINT_HANDLE_UNKNOWN, endpoint, 0,
	...
	/* Get endpoint type.  */
	endpoint_type =
		(endpoint->ux_endpoint_descriptor.bmAttributes) & UX_MASK_ENDPOINT_TYPE;
	...
	if ((endpoint_type == UX_INTERRUPT_ENDPOINT) ||
		...
		if (hcd_stm32->ux_hcd_stm32_periodic_ed_head == ed) {
			...
			if (next_ed) {
				...
				next_ed->ux_stm32_ed_next_ed =
					next_ed->ux_stm32_ed_next_ed->ux_stm32_ed_next_ed;
			}
		}
...
STATIC_UINT _ux_hcd_stm32_endpoint_reset(UX_HCD_STM32 *hcd_stm32, UX_ENDPOINT *endpoint) {
	UX_HCD_STM32_ED *ed;
...
STATIC_UINT _ux_hcd_stm32_frame_number_get(UX_HCD_STM32 *hcd_stm32, ULONG *frame_number) {
	...
	while (ed != UX_NULL) {
		...
		if (hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].do_ssplit == 1U) {
			...
			if (transfer_request != NULL) {
				if ((frame_index & ed->ux_stm32_ed_interval_mask) ==
					ed->ux_stm32_ed_interval_position) {
					hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].ep_ss_schedule =
						1U;
				}

				/* Schedule Start & Complete split where the entire split transaction is
				completely bounded by a frame FS/LS devices */
				if (((((frame_index & 0x7U) < 0x3U) || ((frame_index & 0x7U) == 0x7U)) &&
					 (hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].ep_ss_schedule ==
					  1U)) ||
					((hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].do_csplit ==
					  1U) &&
					...
					if (ed->ux_stm32_ed_sch_mode) {
						/* If it's scheduled each SOF/uSOF, the request should be
						 * submitted immediately after packet is done. This is performed
						 * in callback.  */
						if (ed->ux_stm32_ed_interval_mask == 0)
							ed->ux_stm32_ed_sch_mode = 0;
						...
						/* Prepare transactions.  */
						_ux_hcd_stm32_request_trans_prepare(hcd_stm32, ed,
															transfer_request);

						/* Get the pointer to the Endpoint.  */
						endpoint =
							(UX_ENDPOINT *)transfer_request->ux_transfer_request_endpoint;

						/* Check if device connected to hub  */
						if (endpoint->ux_endpoint_device->ux_device_parent != NULL) {
							parent_device =
								endpoint->ux_endpoint_device->ux_device_parent;
							if (parent_device->ux_device_current_configuration
									->ux_configuration_first_interface
									->ux_interface_descriptor.bInterfaceClass == 0x9U) {
								parent_endpoint =
									parent_device->ux_device_current_configuration
										->ux_configuration_first_interface
										->ux_interface_first_endpoint;

								if (parent_endpoint->ux_endpoint_transfer_request
										.ux_transfer_request_actual_length != 0U) {
									/* The interrupt pipe buffer contains the status
									change for each of the ports the length of the buffer
									can be 1 or 2 depending on the number of ports.
									Usually, since HUBs can be bus powered the maximum
									number of ports is 4. We must be taking precautions on
									how we read the buffer content for big endian
									machines.  */
									if (parent_endpoint->ux_endpoint_transfer_request
											.ux_transfer_request_actual_length == 1)
										port_status_change_bits =
											*(USHORT *)parent_endpoint
												 ->ux_endpoint_transfer_request
												 .ux_transfer_request_data_pointer;
									else
										port_status_change_bits =
											(USHORT)_ux_utility_short_get(
												parent_endpoint
													->ux_endpoint_transfer_request
													.ux_transfer_request_data_pointer);

									if ((port_status_change_bits &
										 (0x1U << endpoint->ux_endpoint_device
													  ->ux_device_port_location)) != 0U) {
										ep_schedule = 0U;
									}
								}
							}
						...
						if ((endpoint->ux_endpoint_device->ux_device_state ==
...
STATIC_UINT _ux_hcd_stm32_port_disable(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Return successful completion.  */
	return (UX_SUCCESS);
}

STATIC_UINT _ux_hcd_stm32_port_enable(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Return successful completion.  */
	return (UX_SUCCESS);
}

STATIC_UINT _ux_hcd_stm32_port_reset(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Check to see if this port is valid on this controller.  On STM32, there is only
	 * one. */
	if (port_index != 0) {
		/* Error trap. */
		_ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_HCD,
		...
		/* If trace is enabled, insert this event into the trace buffer.  */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_PORT_INDEX_UNKNOWN, port_index, 0, 0,
								UX_TRACE_ERRORS, 0, 0)
	...
	if ((hcd_stm32->ux_hcd_stm32_controller_flag &
		...
		/* If trace is enabled, insert this event into the trace buffer.  */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_NO_DEVICE_CONNECTED, port_index, 0, 0,
								UX_TRACE_ERRORS, 0, 0)
...
STATIC_UINT _ux_hcd_stm32_port_resume(UX_HCD_STM32 *hcd_stm32, UINT port_index) {
	/* Return error status.  */
	return (UX_FUNCTION_NOT_SUPPORTED);
}

STATIC_UINT _ux_hcd_stm32_port_status_get(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	ULONG port_status;
...
STATIC_UINT _ux_hcd_stm32_port_suspend(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Return error status.  */
	return (UX_FUNCTION_NOT_SUPPORTED);
}

STATIC_UINT _ux_hcd_stm32_power_down_port(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Return error status.  */
	return (UX_FUNCTION_NOT_SUPPORTED);
}

STATIC_UINT _ux_hcd_stm32_power_on_port(UX_HCD_STM32 *hcd_stm32, ULONG port_index) {
	/* Check to see if this port is valid on this controller.  On STM32, there is only
	 * one. */
	if (port_index != 0) {
		/* Error trap. */
...
STATIC_UINT _ux_hcd_stm32_request_bulk_transfer(UX_HCD_STM32 *hcd_stm32,
										 UX_TRANSFER *transfer_request) {
#if defined(UX_HOST_STANDALONE)
	UX_INTERRUPT_SAVE_AREA
#endif /* defined(UX_HOST_STANDALONE) */
...
#if defined(USBH_HAL_HUB_SPLIT_SUPPORTED)
	if (hcd_stm32->hcd_handle->hc[ed->ux_stm32_ed_channel].do_ssplit == 1U) {
		if ((direction == 0) && (transfer_request->ux_transfer_request_requested_length >
								 endpoint->ux_endpoint_descriptor.wMaxPacketSize)) {
			/* Set transfer length to MPS.  */
...
#endif /* USBH_HAL_HUB_SPLIT_SUPPORTED */
	{
		...
		if (hcd_stm32->hcd_handle->Init.dma_enable) {
			if (transfer_request->ux_transfer_request_requested_length >
				endpoint->ux_endpoint_transfer_request.ux_transfer_request_maximum_length)
				length = endpoint->ux_endpoint_transfer_request
							 .ux_transfer_request_maximum_length;
			else
				length = transfer_request->ux_transfer_request_requested_length;
		} else {
			/* If the direction is OUT, request size is larger than MPS, and DMA is not
			 * used, we need to set transfer length to MPS.  */
			if ((direction == 0) &&
				(transfer_request->ux_transfer_request_requested_length >
				 endpoint->ux_endpoint_descriptor.wMaxPacketSize)) {
				/* Set transfer length to MPS.  */
	...
	ed->ux_stm32_ed_status =
		direction == 0 ? UX_HCD_STM32_ED_STATUS_BULK_OUT : UX_HCD_STM32_ED_STATUS_BULK_IN;
	...
	/* Submit the transfer request.  */
	HAL_HCD_HC_SubmitRequest(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel, direction,
							 EP_TYPE_BULK, USBH_PID_DATA, ed->ux_stm32_ed_data, length,
							 0);
...
STATIC_UINT _ux_hcd_stm32_request_control_transfer(UX_HCD_STM32 *hcd_stm32,
											UX_TRANSFER *transfer_request) {
#if defined(UX_HOST_STANDALONE)
	UX_INTERRUPT_SAVE_AREA
#else
	...
	case UX_HCD_STM32_ED_STATUS_CONTROL_SETUP | UX_HCD_STM32_ED_STATUS_TRANSFER_DONE:
		...
		if (ed->ux_stm32_ed_saved_length) {
			...
			/* To status stage.  */
			_ux_hcd_stm32_request_control_status(hcd_stm32, ed, endpoint,
												 transfer_request);
		}
		UX_RESTORE
		return (UX_STATE_WAIT);
	...
	case UX_HCD_STM32_ED_STATUS_CONTROL_DATA_OUT | UX_HCD_STM32_ED_STATUS_TRANSFER_DONE:
		...
		if (ed->ux_stm32_ed_dir) {
			...
			transfer_request->ux_transfer_request_actual_length =
				HAL_HCD_HC_GetXferCount(hcd_stm32->hcd_handle, ed->ux_stm32_ed_channel);
		} else {
	...
	default:
		...
		/* If trace is enabled, insert this event into the trace buffer.  */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_INVALID_STATE, 0, 0, 0,
								UX_TRACE_ERRORS, 0, 0)
	...
	status = _ux_host_semaphore_get(&transfer_request->ux_transfer_request_semaphore,
									MS_TO_TICK(UX_CONTROL_TRANSFER_TIMEOUT));
	...
	if (status != UX_SUCCESS) {
		...
		/* If trace is enabled, insert this event into the trace buffer.  */
		UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_TRANSFER_TIMEOUT, transfer_request, 0,
								0, UX_TRACE_ERRORS, 0, 0)
	...
	if (ed->ux_stm32_ed_saved_length) {
		...
		/* If the semaphore did not succeed we probably have a time out.  */
		if (status != UX_SUCCESS) {
			/* All transfers pending need to abort. There may have been a partial
			 * transfer.  */
			_ux_host_stack_transfer_request_abort(transfer_request);
			...
			/* If trace is enabled, insert this event into the trace buffer.  */
			UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_TRANSFER_TIMEOUT, transfer_request,
...
STATIC_UINT _ux_hcd_stm32_request_periodic_transfer(UX_HCD_STM32 *hcd_stm32,
											 UX_TRANSFER *transfer_request) {
	UX_ENDPOINT *endpoint;
	UX_HCD_STM32_ED *ed;
	UX_TRANSFER *transfer;
...
STATIC_UINT _ux_hcd_stm32_request_transfer(UX_HCD_STM32 *hcd_stm32,
									UX_TRANSFER *transfer_request) {
	UX_ENDPOINT *endpoint;
	UINT status;
...
STATIC_UINT _ux_hcd_stm32_transfer_abort(UX_HCD_STM32 *hcd_stm32,
								  UX_TRANSFER *transfer_request) {
	UX_ENDPOINT *endpoint;
	UX_HCD_STM32_ED *ed;
	UX_TRANSFER *transfer;
