...
#ifndef SUBSYS_SD_MMC_H_
#define SUBSYS_SD_MMC_H_
 ...
 */
int mmc_write_blocks(struct sd_card *card, const uint8_t *wbuf, uint32_t start_block,
					 uint32_t num_blocks);
 ...
 * @retval -EIO: I/O error
 */
int mmc_read_blocks(struct sd_card *card, uint8_t *rbuf, uint32_t start_block,
					uint32_t num_blocks);
