...
static int sdio_send_ocr(struct sd_card *card, uint32_t ocr) {
	...
	for (retries = 0; retries < CONFIG_SD_OCR_RETRY_COUNT; retries++) {
		...
		if (ocr == 0) {
			...
			card->num_io =
				(cmd.response[0] & SDIO_OCR_IO_NUMBER) >> SDIO_OCR_IO_NUMBER_SHIFT;
	...
	if (retries >= CONFIG_SD_OCR_RETRY_COUNT) {
		/* OCR timed out */
		pr_err("Card never left busy state\n");
		return -ETIMEDOUT;
	}
	pr_dbg("SDIO responded to CMD5 after %d attempts\n", retries);
...
static int sdio_io_rw_direct(struct sd_card *card, enum sdio_io_dir direction,
							 enum sdio_func_num func, uint32_t reg_addr, uint8_t data_in,
							 uint8_t *data_out) {
	int ret;
	struct sdhc_command cmd = {0};
...
static int sdio_io_rw_extended(struct sd_card *card, enum sdio_io_dir direction,
							   enum sdio_func_num func, uint32_t reg_addr, bool increment,
							   uint8_t *buf, uint32_t blocks, uint32_t block_size) {
	struct sdhc_command cmd = {0};
 ...
 */
static int sdio_io_rw_extended_helper(struct sdio_func *func, enum sdio_io_dir direction,
									  uint32_t reg_addr, bool increment, uint8_t *buf,
									  uint32_t len) {
	int ret;
	int remaining = len;
	...
	if ((func->card->cccr_flags & SDIO_SUPPORT_MULTIBLOCK) &&
		...
		while (remaining >= func->block_size) {
			...
			ret = sdio_io_rw_extended(func->card, direction, func->num, reg_addr,
									  increment, buf, blocks, func->block_size);
			if (ret) {
				return ret;
			}
	...
	while (remaining > 0) {
		size = rte_min_t(uint32_t, remaining, func->cis.max_blk_size);

		ret = sdio_io_rw_extended(func->card, direction, func->num, reg_addr, increment,
								  buf, 0, size);
		if (ret) {
			return ret;
		}
...
static int sdio_read_cccr(struct sd_card *card) {
	...
	ret =
		sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_CCCR, 0, &data);
	if (ret) {
		pr_dbg("CCCR read failed: %d\n", ret);
		return ret;
	}
	cccr_ver = (data & SDIO_CCCR_CCCR_REV_MASK) >> SDIO_CCCR_CCCR_REV_SHIFT;
	pr_dbg("SDIO cccr revision %u\n", cccr_ver);
	/* Read SD spec version */
	ret = sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_SD, 0, &data);
	if (ret) {
		return ret;
	}
	...
	ret =
		sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_CAPS, 0, &data);
	if (ret) {
		return ret;
	}
	card->cccr_flags = 0;
	...
	if (cccr_ver >= SDIO_CCCR_CCCR_REV_2_00) {
		...
		ret = sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_SPEED, 0,
								&data);
		if (ret) {
			return ret;
		}
	...
	if (cccr_ver >= SDIO_CCCR_CCCR_REV_3_00 && (card->flags & SD_1800MV_FLAG)) {
		/* Read UHS properties */
		ret = sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_UHS, 0,
								&data);
		if (ret) {
			return ret;
		}
...
static void sdio_decode_cis(struct sdio_cis *cis, enum sdio_func_num func, uint8_t *data,
							uint8_t tpl_code, uint8_t tpl_link) {
	switch (tpl_code) {
 ...
 */
static int sdio_read_cis(struct sdio_func *func, uint8_t *tuples, uint32_t tuple_count) {
	int ret;
	uint8_t *data = func->card->card_buffer;
	...
	do {
		/* Read tuple code */
		ret = sdio_io_rw_direct(func->card, SDIO_IO_READ, SDIO_FUNC_NUM_0, cis_ptr++, 0,
								&tpl_code);
		if (ret) {
			return ret;
		}
		...
		for (int i = 0; i < (int)tuple_count; i++) {
		...
		if (match_tpl) {
			...
			sdio_decode_cis(&func->cis, func->num, data, tpl_code, tpl_link);
		} else {
			/* Advance CIS pointer */
			cis_ptr += tpl_link;
		}
	} while (num < tuple_count);
	pr_dbg("SDIO CIS max block size for func %d: %d\n", func->num,
...
static int sdio_set_bus_width(struct sd_card *card, enum sdhc_bus_width width) {
	...
	ret = sdio_io_rw_direct(card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_BUS_IF, 0,
							&reg_bus_interface);
	if (ret) {
		return ret;
	}
	...
	ret = sdio_io_rw_direct(card, SDIO_IO_WRITE, SDIO_FUNC_NUM_0, SDIO_CCCR_BUS_IF,
							reg_bus_interface, &reg_bus_interface);
	if (ret) {
		return ret;
	}
	...
	if (ret) {
		pr_dbg("Could not change host bus width\n");
	}
	return ret;
}
...
static int sdio_set_bus_speed(struct sd_card *card) {
	...
	case SD_TIMING_SDR104:
		bus_clock =
			rte_min_t(uint32_t, card->host_props.f_max, card->switch_caps.uhs_max_dtr);
	...
	case SD_TIMING_HIGH_SPEED:
		bus_clock =
			rte_min_t(uint32_t, card->host_props.f_max, card->switch_caps.hs_max_dtr);
		target_speed = SDIO_CCCR_SPEED_SDR25;
		timing = SDHC_TIMING_HS;
		break;
	case SD_TIMING_DEFAULT:
		bus_clock = rte_min_t(uint32_t, card->host_props.f_max, _SD_SPEC_MHZ(25));
	...
	do {
		...
		ret = sdio_io_rw_direct(card, SDIO_IO_WRITE, SDIO_FUNC_NUM_0, SDIO_CCCR_SPEED,
								speed_reg, &speed_reg);
		if (ret) {
			return ret;
		}
	...
	if (retries == 0) {
		...
		pr_warn("Could not set target SDIO speed\n");
	} else {
		/* Set card bus clock and timing */
		card->bus_io.timing = timing;
		card->bus_io.clock = bus_clock;
		pr_dbg("Setting bus clock to: %d\n", card->bus_io.clock);
		ret = sdhc_set_io(card->sdhc, &card->bus_io);
		if (ret) {
			pr_err("Failed to change host bus speed\n");
			return ret;
		}
	}
	return ret;
...
int sdio_card_init(struct sd_card *card) {
	...
	if (RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_NATIVE_MODE) &&
	...
	if (card->ocr & SD_OCR_SWITCH_18_ACCEPT_FLAG) {
		pr_dbg("Card supports 1.8V signalling\n");
	...
	if (RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_NATIVE_MODE)) {
		 ...
		 */
		if ((card->flags & SD_1800MV_FLAG) && (!card->host_props.is_spi) &&
			(card->host_props.host_caps.vol_180_support) &&
			RTE_IS_ENABLED(CONFIG_SD_UHS_PROTOCOL)) {
		...
		if ((card->flags & SD_MEM_PRESENT_FLAG) && ((card->flags & SD_SDHC_FLAG) == 0)) {
	...
	ret = sdio_read_cis(&card->func0, cis_tuples, rte_array_size(cis_tuples));
	if (ret) {
		return ret;
	}
	...
	if (RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_NATIVE_MODE) &&
		...
		pr_dbg("Raised card bus width to 4 bits\n");
	}
...
int sdio_init_func(struct sd_card *card, struct sdio_func *func, enum sdio_func_num num) {
	...
	return sdio_read_cis(func, cis_tuples, rte_array_size(cis_tuples));
}
...
int sdio_enable_func(struct sdio_func *func) {
	...
	ret = sdio_io_rw_direct(func->card, SDIO_IO_READ, SDIO_FUNC_NUM_0, SDIO_CCCR_IO_EN, 0,
							&reg);
	if (ret) {
		return ret;
	}
	reg |= BIT(func->num);
	ret = sdio_io_rw_direct(func->card, SDIO_IO_WRITE, SDIO_FUNC_NUM_0, SDIO_CCCR_IO_EN,
							reg, &reg);
	if (ret) {
		return ret;
	}
...
int sdio_set_block_size(struct sdio_func *func, uint16_t bsize) {
	...
	for (int i = 0; i < 2; i++) {
		...
		ret = sdio_io_rw_direct(func->card, SDIO_IO_WRITE, SDIO_FUNC_NUM_0,
								SDIO_FBR_BASE(func->num) + SDIO_FBR_BLK_SIZE + i, reg,
								NULL);
		if (ret) {
			return ret;
		}
...
int sdio_read_byte(struct sdio_func *func, uint32_t reg, uint8_t *val) {
	...
	if ((func->card->type != CARD_SDIO) && (func->card->type != CARD_COMBO)) {
		pr_warn("Card does not support SDIO commands\n");
		return -ENOTSUP;
	}
	ret = tx_mutex_get(&func->card->lock, TX_MSEC(CONFIG_SD_DATA_TIMEOUT));
	if (ret) {
		pr_warn("Could not get SD card mutex\n");
		return -EBUSY;
	}
	ret = sdio_io_rw_direct(func->card, SDIO_IO_READ, func->num, reg, 0, val);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_write_byte(struct sdio_func *func, uint32_t reg, uint8_t write_val) {
	...
	ret = sdio_io_rw_direct(func->card, SDIO_IO_WRITE, func->num, reg, write_val, NULL);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_rw_byte(struct sdio_func *func, uint32_t reg, uint8_t write_val,
	...
	ret =
		sdio_io_rw_direct(func->card, SDIO_IO_WRITE, func->num, reg, write_val, read_val);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_read_fifo(struct sdio_func *func, uint32_t reg, uint8_t *data, uint32_t len) {
	...
	ret = sdio_io_rw_extended_helper(func, SDIO_IO_READ, reg, false, data, len);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_write_fifo(struct sdio_func *func, uint32_t reg, uint8_t *data, uint32_t len) {
	...
	ret = sdio_io_rw_extended_helper(func, SDIO_IO_WRITE, reg, false, data, len);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_read_blocks_fifo(struct sdio_func *func, uint32_t reg, uint8_t *data,
	...
	ret = sdio_io_rw_extended(func->card, SDIO_IO_READ, func->num, reg, false, data,
...
int sdio_write_blocks_fifo(struct sdio_func *func, uint32_t reg, uint8_t *data,
	...
	ret = sdio_io_rw_extended(func->card, SDIO_IO_WRITE, func->num, reg, false, data,
							  blocks, func->block_size);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_read_addr(struct sdio_func *func, uint32_t reg, uint8_t *data, uint32_t len) {
	...
	ret = sdio_io_rw_extended_helper(func, SDIO_IO_READ, reg, true, data, len);
	tx_mutex_put(&func->card->lock);
	return ret;
}
...
int sdio_write_addr(struct sdio_func *func, uint32_t reg, uint8_t *data, uint32_t len) {
	...
	ret = sdio_io_rw_extended_helper(func, SDIO_IO_WRITE, reg, true, data, len);
	tx_mutex_put(&func->card->lock);
	return ret;
}
