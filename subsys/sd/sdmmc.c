...
static inline void sdmmc_decode_scr(struct sd_scr *scr, uint32_t *raw_scr,
									uint8_t *version) {
...
static int sdmmc_spi_send_ocr(struct sd_card *card, uint32_t arg) {
	...
	if (ret) {
		pr_dbg("CMD58 failed: %d\n", ret);
		return ret;
	}

	card->ocr = cmd.response[1];
	if (card->ocr == 0) {
		pr_dbg("No OCR detected\n");
		return -ENOTSUP;
	}
...
static int sdmmc_send_ocr(struct sd_card *card, int ocr) {
	...
	pr_dbg("SDMMC responded to ACMD41 after %d attempts\n", retries);
	if (!card->host_props.is_spi) {
		/* Save OCR */
...
static int sdmmc_read_scr(struct sd_card *card) {
	...
	if (ret) {
		pr_dbg("SD app command failed for SD SCR\n");
		return ret;
	}
	...
	if (ret) {
		pr_dbg("ACMD51 failed: %d\n", ret);
		return ret;
	}
	/* Decode SCR */
	raw_scr[0] = rte_be32_to_cpu(scr[0]);
	raw_scr[1] = rte_be32_to_cpu(scr[1]);
	sdmmc_decode_scr(&card_scr, raw_scr, &card->sd_version);
	pr_dbg("SD reports specification version %d\n", card->sd_version);
...
static int sdmmc_set_bus_width(struct sd_card *card, enum sdhc_bus_width width) {
	...
	ret = sdmmc_app_command(card, card->relative_addr);
	if (ret) {
		pr_dbg("SD app command failed for ACMD6\n");
		return ret;
	}
	...
	ret = sdhc_request(card->sdhc, &cmd, NULL);
	if (ret) {
		pr_dbg("Error on ACMD6: %d\n", ret);
		return ret;
	}
	ret = sd_check_response(&cmd);
	if (ret) {
		pr_dbg("ACMD6 reports error, response 0x%x\n", cmd.response[0U]);
		return ret;
	}
...
static int sdmmc_read_switch(struct sd_card *card) {
	...
	if (card->sd_version < SD_SPEC_VER1_1) {
		...
		pr_info("SD spec 1.01 does not support CMD6\n");
		return 0;
	}
...
static inline void sdmmc_select_bus_speed(struct sd_card *card) {
	...
	if ((card->flags & SD_1800MV_FLAG) && sdmmc_host_uhs(&card->host_props) &&
		!(card->host_props.is_spi) && RTE_IS_ENABLED(CONFIG_SD_UHS_PROTOCOL)) {
...
static int sdmmc_set_bus_speed(struct sd_card *card) {
	...
	if ((card->flags & SD_1800MV_FLAG) && sdmmc_host_uhs(&card->host_props) &&
		!(card->host_props.is_spi) && RTE_IS_ENABLED(CONFIG_SD_UHS_PROTOCOL)) {
		/* UHS mode */
		card_clock = rte_min_t(uint32_t, card->host_props.f_max, card->switch_caps.uhs_max_dtr);
	...
	if (ret) {
		pr_dbg("Failed to switch SD card speed\n");
		return ret;
	}
	if ((status[16] & 0xF) != card->card_speed) {
		pr_warn("Card did not accept new speed\n");
	} else {
...
static int sdmmc_init_uhs(struct sd_card *card) {
	...
	if (ret) {
		pr_err("Failed to change card bus width to 4 bits\n");
		return ret;
	}
	...
	if (ret) {
		pr_dbg("Failed to select new driver type\n");
		return ret;
	}
	ret = sdmmc_set_current_limit(card);
	if (ret) {
		pr_dbg("Failed to set card current limit\n");
		return ret;
	}
	/* Apply the bus speed selected earlier */
	ret = sdmmc_set_bus_speed(card);
	if (ret) {
		pr_dbg("Failed to set card bus speed\n");
		return ret;
	}
	if (card->card_speed == SD_TIMING_SDR50 || card->card_speed == SD_TIMING_SDR104 ||
		card->card_speed == SD_TIMING_DDR50) {
...
static int sdmmc_init_hs(struct sd_card *card) {
	...
	ret = sdmmc_set_bus_speed(card);
	if (ret) {
		pr_err("Failed to switch card to HS mode\n");
		return ret;
	}
...
int sdmmc_card_init(struct sd_card *card) {
	 ...
	 * probing OCR
	 */
	tx_os_nanosleep(100000);
	
	/* Send SD OCR to card to initialize it */
	ret = sdmmc_send_ocr(card, ocr_arg);
	if (ret) {
		pr_err("Failed to query card OCR\n");
		return ret;
	}
	if (card->host_props.is_spi && RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_SPI_MODE)) {
	...
	if (card->ocr & SD_OCR_SWITCH_18_ACCEPT_FLAG) {
		pr_dbg("Card supports 1.8V signaling\n");
	...
	if (!card->host_props.is_spi && RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_NATIVE_MODE)) {
		/*
	...
	if (card->host_props.f_max < SD_CLOCK_25MHZ) {
		pr_info("Maximum SD clock is under 25MHz, using clock of %dHz\n",
	...
	ret = sdhc_set_io(card->sdhc, &card->bus_io);
	if (ret) {
		pr_err("Failed to raise bus frequency to 25MHz\n");
		return ret;
	}
	...
	if (!card->host_props.is_spi && RTE_IS_ENABLED(CONFIG_SDHC_SUPPORTS_NATIVE_MODE)) {
		...
		if (ret) {
			pr_err("Failed to read card functions\n");
			return ret;
		}
	}
	if ((card->flags & SD_1800MV_FLAG) && sdmmc_host_uhs(&card->host_props) &&
		...
		if ((card->flags & SD_HIGH_CAPACITY_FLAG) == 0) {
			...
			if (ret) {
				pr_err("Could not set SD blocklen to 512\n");
				return ret;
			}
			card->block_size = 512;
		}
