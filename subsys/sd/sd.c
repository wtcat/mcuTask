...
static int sd_send_interface_condition(struct sd_card *card) {
	...
	if (ret) {
		pr_err("SD CMD8 failed with error %d\n", ret);
		/* Retry */
		return SD_RETRY;
	}
	...
	if ((resp & 0xFF) != SD_IF_COND_CHECK) {
		pr_info("Legacy card detected, no CMD8 support\n");
		/* Retry probe */
		return SD_RETRY;
	}
	...
	pr_dbg("Found SDHC with CMD8 support\n");
...
static int sd_enable_crc(struct sd_card *card) {
	...
	rte_assert(card->host_props.is_spi);
...
static int sd_common_init(struct sd_card *card) {
	...
	ret = sd_idle(card);
	if (ret) {
		pr_err("Card error on CMD0\n");
		return ret;
	}
	/* Perform voltage check using SD CMD8 */
	ret = sd_retry(sd_send_interface_condition, card, CONFIG_SD_RETRY_COUNT);
	if (ret == -ETIMEDOUT) {
		pr_info("Card does not support CMD8, assuming legacy card\n");
		return sd_idle(card);
	} else if (ret) {
		pr_err("Card error on CMD 8\n");
		return ret;
	}
...
static int sd_init_io(struct sd_card *card) {
	...
	if (host_props->host_caps.vol_330_support) {
		pr_dbg("Host controller support 3.3V max\n");
		voltage = SD_VOL_3_3_V;
	} else if (host_props->host_caps.vol_300_support) {
		pr_dbg("Host controller support 3.0V max\n");
		voltage = SD_VOL_3_0_V;
	} else {
		pr_dbg("Host controller support 1.8V max\n");
	...
	pr_dbg("Resetting power to card\n");
	bus_io->power_mode = SDHC_POWER_OFF;
	ret = sdhc_set_io(card->sdhc, bus_io);
	if (ret) {
		pr_err("Could not disable card power via SDHC\n");
		return ret;
	}
	...
	ret = sdhc_set_io(card->sdhc, bus_io);
	if (ret) {
		pr_err("Could not start bus clock\n");
		return ret;
	}
	return 0;
}
...
int sd_init(struct device *sdhc_dev, struct sd_card *card) {
	int ret;
	...
	if (ret) {
		pr_err("SD host controller returned invalid properties\n");
		return ret;
	}

	/* init and lock card mutex */
	ret = tx_mutex_create(&card->lock, "sd_card", TX_INHERIT);
	if (ret) {
		pr_dbg("Could not init card mutex\n");
		return ret;
	}
	ret = tx_mutex_get(&card->lock, TX_MSEC(CONFIG_SD_INIT_TIMEOUT));
	if (ret) {
		pr_err("Timeout while trying to acquire card mutex\n");
		return ret;
	}
	...
	if (ret == SD_RESTART) {
		...
		ret = sd_init_io(card);
		if (ret) {
			pr_err("Failed to reset SDHC I/O\n");
			tx_mutex_put(&card->lock);
			return ret;
		}
		ret = sd_command_init(card);
		if (ret) {
			pr_err("Failed to init SD card after I/O reset\n");
			tx_mutex_put(&card->lock);
			return ret;
		}
	} else if (ret != 0) {
	...
	if (ret) {
		pr_dbg("Could not unlock card mutex\n");
		return ret;
	}
	return ret;
}
...
bool sd_is_card_present(struct device *sdhc_dev) {
	if (!sdhc_dev) {
		return false;
	}
