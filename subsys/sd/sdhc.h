...
#ifndef SUBSYS_SD_SDHC_H_
#define SUBSYS_SD_SDHC_H_
...
struct sdhc_host_caps {
	...
	unsigned int
		address_64_bit_support_v4 : 1; /**< 64-bit system address support for V4 */
	unsigned int
		address_64_bit_support_v3 : 1; /**< 64-bit system address support for V3 */
...
struct sdhc_driver_api {
	...
	int (*set_io)(struct device *dev, struct sdhc_io *ios);
	int (*get_card_present)(struct device *dev);
	int (*execute_tuning)(struct device *dev);
	int (*card_busy)(struct device *dev);
	int (*get_host_props)(struct device *dev, struct sdhc_host_props *props);
