...
#define MMC_SWITCH_8_BIT_DDR_BUS_ARG                                                     \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (183U << 16)) + (0x0000FF00 & (6U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_8_BIT_BUS_ARG                                                         \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (183U << 16)) + (0x0000FF00 & (2U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_4_BIT_BUS_ARG                                                         \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (183U << 16)) + (0x0000FF00 & (1U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_HS_TIMING_ARG                                                         \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (185U << 16)) + (0x0000FF00 & (1U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_HS400_TIMING_ARG                                                      \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (185U << 16)) + (0x0000FF00 & (3U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_HS200_TIMING_ARG                                                      \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (185U << 16)) + (0x0000FF00 & (2U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_RCA_ARG (CONFIG_MMC_RCA << 16U)
#define MMC_REL_ADR_ARG (card->relative_addr << 16U)
#define MMC_SWITCH_PWR_CLASS_ARG                                                         \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (187U << 16)) + (0x0000FF00 & (0U << 8)) +                         \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
#define MMC_SWITCH_CACHE_ON_ARG                                                          \
	(0xFC000000 & (0U << 26)) + (0x03000000 & (0b11 << 24)) +                            \
		(0x00FF0000 & (33U << 16)) + (0x0000FF00 & (1U << 8)) +                          \
		(0x000000F7 & (0U << 3)) + (0x00000000 & (3U << 0))
...
int mmc_card_init(struct sd_card *card) {
	...
	if (ret) {
		pr_err("Failed on sending RCA to card\n");
		return ret;
	}
...
static int mmc_send_op_cond(struct sd_card *card, int ocr) {
	...
	pr_dbg("MMC responded to CMD1 after %d attempts\n", retries);
	...
	if (card->host_props.host_caps.vol_180_support && (card->flags & SD_1800MV_FLAG)) {
		...
		if (ret) {
			pr_dbg("Failed to switch MMC host to 1.8V\n");
			return ret;
		}
		sd_delay(10);
...
static inline int mmc_set_max_freq(struct sd_card *card, struct sd_csd *card_csd) {
	...
	/* 4.0 - 4.2 emmc spec says 20 MHz */
		...
		pr_info("Using Legacy MMC will have slow initialization\n");
		return 0;
	}

	ret = sdhc_set_io(card->sdhc, &card->bus_io);
	if (ret) {
		pr_err("Error seetting initial clock frequency\n");
		return ret;
	}
...
static int mmc_set_bus_width(struct sd_card *card) {
	...
	if (ret) {
		pr_err("Setting card data bus width failed: %d\n", ret);
		return ret;
	}
	...
	ret = sdhc_set_io(card->sdhc, &card->bus_io);
	if (ret) {
		pr_err("Setting SDHC data bus width failed: %d\n", ret);
		return ret;
	}
...
static int mmc_set_hs_timing(struct sd_card *card) {
	...
	ret = sdhc_request(card->sdhc, &cmd, NULL);
	if (ret) {
		pr_dbg("Error setting bus timing: %d\n", ret);
		return ret;
	}
	sdmmc_wait_ready(card);
...
static int mmc_set_timing(struct sd_card *card, struct mmc_ext_csd *ext) {
	...
	if ((ext->device_type.MMC_HS200_SDR_1200MV ||
		 ext->device_type.MMC_HS200_SDR_1800MV) &&
	...
	if (card->card_speed == MMC_HS200_TIMING) {
		...
		if (ret) {
			pr_err("MMC Tuning failed: %d\n", ret);
			return ret;
		}
	}

	/* Switch to HS400 if applicable */
	if ((ext->device_type.MMC_HS400_DDR_1200MV ||
		 ext->device_type.MMC_HS400_DDR_1800MV) &&
		(card->host_props.host_caps.hs400_support) &&
		(card->bus_io.bus_width == SDHC_BUS_WIDTH8BIT)) {
		/* Switch back to regular HS timing */
		ret = mmc_set_hs_timing(card);
		if (ret) {
			pr_err("Switching MMC back to HS from HS200 during HS400 init failed.\n");
			return ret;
		}
		...
		sdmmc_wait_ready(card);
		if (ret) {
			pr_err("Setting DDR data bus width failed during HS400 init: %d\n", ret);
			return ret;
		}
		...
		ret = sdhc_request(card->sdhc, &cmd, NULL);
		if (ret) {
			pr_dbg("Error setting card to HS400 bus timing: %d\n", ret);
			return ret;
		}
		ret = sdmmc_wait_ready(card);
		if (ret) {
...
static int mmc_read_ext_csd(struct sd_card *card, struct mmc_ext_csd *card_ext_csd) {
	...
	if (ret) {
		pr_err("CMD8 (send_ext_csd) failed: %d\n", ret);
		return ret;
	}
	...
	pr_info("Card block count is %d, block size is %d\n", card->block_count,
			card->block_size);
...
static int mmc_set_cache(struct sd_card *card, struct mmc_ext_csd *card_ext_csd) {
	...
	ret = sdhc_request(card->sdhc, &cmd, NULL);
	if (ret) {
		pr_dbg("Error turning on card cache: %d\n", ret);
		return ret;
	}
	ret = sdmmc_wait_ready(card);
	return ret;
