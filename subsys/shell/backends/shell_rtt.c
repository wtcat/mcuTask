...
#define RTT_LOCK() COND_CODE_0(CONFIG_SHELL_BACKEND_RTT_BUFFER, (SEGGER_RTT_LOCK()), ())
...
SHELL_DEFINE(shell_rtt, CONFIG_SHELL_PROMPT_RTT, &shell_transport_rtt,
			 ...
			 CONFIG_SHELL_BACKEND_RTT_LOG_MESSAGE_QUEUE_TIMEOUT, SHELL_FLAG_OLF_CRLF);
...
static inline bool is_sync_mode(void) {
	return (IS_ENABLED(CONFIG_LOG_MODE_IMMEDIATE) &&
			IS_ENABLED(CONFIG_SHELL_LOG_BACKEND)) ||
		   is_panic_mode();
}
...
static void on_failed_write(int retry_cnt) {
	if (retry_cnt == 0) {
		...
		k_busy_wait(USEC_PER_MSEC * CONFIG_SHELL_BACKEND_RTT_RETRY_DELAY_MS);
	} else {
...
static void on_write(int retry_cnt) {
	...
	if (is_panic_mode()) {
		 ...
		 */
		while (SEGGER_RTT_HasDataUp(CONFIG_SHELL_BACKEND_RTT_BUFFER) && host_present) {
...
static int write(const struct shell_transport *transport, const void *data, size_t length,
	...
	do {
		if (!is_sync_mode()) {
			RTT_LOCK();
			ret =
				SEGGER_RTT_WriteSkipNoLock(CONFIG_SHELL_BACKEND_RTT_BUFFER, data, length);
			RTT_UNLOCK();
		} else {
			ret =
				SEGGER_RTT_WriteSkipNoLock(CONFIG_SHELL_BACKEND_RTT_BUFFER, data, length);
...
static int read(const struct shell_transport *transport, void *data, size_t length,
				size_t *cnt) {
