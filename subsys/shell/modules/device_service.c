...
static const char *get_device_name(const struct device *dev, char *buf, size_t len) {
...
static int cmd_device_list_visitor(const struct device *dev, void *context) {
...
static int cmd_device_pm_toggle(const struct shell *sh, size_t argc, char **argv) {
	const struct device *dev;
	...
	if (!pm_device_runtime_is_enabled(dev)) {
		shell_error(sh, "Device (%s) does not have runtime power management", argv[1]);
		return -ENOTSUP;
	}
...
#define PM_SHELL_CMD                                                                     \
	SHELL_CMD(pm_toggle, NULL, "Toggle device power (pm get/put)", cmd_device_pm_toggle),
#else
#define PM_SHELL_CMD
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_device,
							   SHELL_CMD(list, NULL, "List configured devices",
										 cmd_device_list),
							   PM_SHELL_CMD SHELL_SUBCMD_SET_END /* Array terminated. */
);
