...
static void formatted_text_print(const struct shell *sh, const char *str,
	...
	while (true) {
		...
		if (length <= sh->ctx->vt100_ctx.cons.terminal_wid - terminal_offset) {
		 ...
		 */
		length = sh->ctx->vt100_ctx.cons.terminal_wid - terminal_offset;
		...
		while (true) {
			...
			if ((idx + terminal_offset) >= sh->ctx->vt100_ctx.cons.terminal_wid) {
				/* End of line reached. */
				break;
			}
...
static void help_item_print(const struct shell *sh, const char *item_name,
	...
	if (!IS_ENABLED(CONFIG_NEWLIB_LIBC) && !IS_ENABLED(CONFIG_ARCH_POSIX)) {
		/* print option name */
		z_shell_fprintf(sh, SHELL_NORMAL, "%s%-*s", tabulator, item_name_width,
						item_name);
	} else {
		uint16_t tmp = item_name_width - strlen(item_name);
		char space = ' ';

		z_shell_fprintf(sh, SHELL_NORMAL, "%s%s", tabulator, item_name);
