...
void z_shell_op_cursor_position_synchronize(const struct shell *sh) {
	...
	z_shell_multiline_data_calc(cons, sh->ctx->cmd_buff_pos, sh->ctx->cmd_buff_len);
...
void z_shell_op_cursor_move(const struct shell *sh, int16_t val) {
	...
	row_span = z_row_span_with_buffer_offsets_get(&sh->ctx->vt100_ctx.cons,
												  sh->ctx->cmd_buff_pos, new_pos);
	col_span = z_column_span_with_buffer_offsets_get(&sh->ctx->vt100_ctx.cons,
													 sh->ctx->cmd_buff_pos, new_pos);
...
static uint16_t shift_calc(const char *str, uint16_t pos, uint16_t len, int16_t sign) {
	...
	while (1) {
		idx = pos + ret * sign;
		if (((idx == 0U) && (sign < 0)) || ((idx == len) && (sign > 0))) {
			break;
		}
...
void z_shell_op_cursor_word_move(const struct shell *sh, int16_t val) {
	...
	while (val--) {
		shift = shift_calc(sh->ctx->cmd_buff, sh->ctx->cmd_buff_pos,
...
void z_shell_op_word_remove(const struct shell *sh) {
	...
	if ((sh->ctx->cmd_buff_len == 0) || (sh->ctx->cmd_buff_pos == 0)) {
		return;
	}
	...
	/* Manage the buffer. */
	memmove(str + 1, str + 1 + chars_to_delete, sh->ctx->cmd_buff_len - chars_to_delete);
	...
	if (z_flag_obscure_get(sh)) {
		...
		if ((buff_pos / sh->ctx->vt100_ctx.cons.terminal_wid) !=
			...
			while (buff_pos < line_len) {
				...
				z_shell_fprintf(sh, SHELL_NORMAL, "%c", sh->ctx->cmd_buff[pos++]);
			}
		} else {
...
void z_shell_op_char_insert(const struct shell *sh, char data) {
	if (z_flag_insert_mode_get(sh) && (sh->ctx->cmd_buff_len != sh->ctx->cmd_buff_pos)) {
...
void z_shell_op_completion_insert(const struct shell *sh, const char * compl,
								  uint16_t compl_len) {
	data_insert(sh, compl, compl_len);
}

void z_shell_cmd_line_erase(const struct shell *sh) {
	z_shell_multiline_data_calc(&sh->ctx->vt100_ctx.cons, sh->ctx->cmd_buff_pos,
								sh->ctx->cmd_buff_len);
	z_shell_op_cursor_horiz_move(sh, -(sh->ctx->vt100_ctx.cons.cur_x - 1));
...
void z_shell_print_cmd(const struct shell *sh) {
	...
	if (cmd_width > 0) {
		z_shell_raw_fprintf(sh->fprintf_ctx, "%s", &sh->ctx->cmd_buff[beg_offset]);
	}
}
...
static void shell_pend_on_txdone(const struct shell *sh) {
	if (IS_ENABLED(CONFIG_MULTITHREADING) &&
		...
		ULONG events;
		tx_event_flags_get(&sh->ctx->event_group, SHELL_SIGNAL_TXDONE, TX_OR_CLEAR,
...
void z_shell_write(const struct shell *sh, const void *data, size_t length) {
	...
	while (length) {
		int err = sh->iface->api->write(sh->iface, &((const uint8_t *)data)[offset],
										length, &tmp_cnt);
		(void)err;
		rte_assert(err == 0);
		rte_assert(length >= tmp_cnt);
		offset += tmp_cnt;
		length -= tmp_cnt;
		if (tmp_cnt == 0 && (sh->ctx->state != SHELL_STATE_PANIC_MODE_ACTIVE)) {
...
static void vt100_bgcolor_set(const struct shell *sh, enum shell_vt100_color bgcolor) {
	...
	if ((bgcolor == SHELL_NORMAL) || (sh->ctx->vt100_ctx.col.bgcol == bgcolor)) {
		return;
	}
...
void z_shell_vt100_color_set(const struct shell *sh, enum shell_vt100_color color) {
	if (!IS_ENABLED(CONFIG_SHELL_VT100_COLORS)) {
		return;
	}
