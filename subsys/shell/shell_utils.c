...
#define Z_SHELL_SUBCMD_END_MARKER_CREATE()                                               \
	static /*const*/ TYPE_SECTION_ITERABLE(struct shell_static_entry,                    \
										   z_shell_subcmd_end_marker, shell_subcmds,     \
										   Z_SHELL_UNDERSCORE(999))
...
void z_shell_multiline_data_calc(struct shell_multiline_cons *cons, uint16_t buff_pos,
								 uint16_t buff_len) {
...
char z_shell_make_argv(size_t *argc, const char **argv, char *cmd, uint8_t max_argc) {
	char quote = 0;
	char c;
...
z_shell_get_last_command(const struct shell_static_entry *entry, size_t argc,
...
static bool shell_device_iterator(struct device *dev, void *arg) {
	struct iterator_param *p = (struct iterator_param *)arg;
	if ((dev->name != NULL) && (strlen(dev->name) != 0) &&
		((p->prefix == NULL) ||
		 (strncmp(p->prefix, dev->name, strlen(p->prefix)) == 0)) &&
		(p->filter == NULL || p->filter(dev))) {
		if (p->match_idx == p->idx) {
			p->dev = dev;
			return true;
		}
		++p->match_idx;
...
static const struct device *shell_device_internal(size_t idx, const char *prefix,
												  shell_device_filter_t filter) {
	struct iterator_param param = {
		.filter = filter,
		.prefix = prefix,
		.dev = NULL,
		.match_idx = 0,
	...
	device_foreach(shell_device_iterator, &param);
	return param.dev;
}
