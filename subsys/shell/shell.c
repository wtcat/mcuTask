...
#include <subsys/shell/shell_dummy.h>
#endif
...
#define SHELL_THREAD_PRIORITY CONFIG_SHELL_THREAD_PRIORITY

BUILD_ASSERT(SHELL_THREAD_PRIORITY <= CONFIG_TX_MAX_PRIORITIES,
			 "Invalid range for thread priority");

struct shell_threadparam {
	void *shell;
	uint32_t log_level;
	bool log_backend;
};
...
static void shell_internal_help_print(const struct shell *sh) {
	...
	z_shell_help_subcmd_print(sh, &sh->ctx->active_cmd, "Subcommands:\n");
}
 ...
 */
static int cmd_precheck(const struct shell *sh, bool arg_cnt_ok) {
	if (!arg_cnt_ok) {
...
static inline void state_set(const struct shell *sh, enum shell_state state) {
	...
	if (state == SHELL_STATE_ACTIVE && !sh->ctx->bypass) {
		...
		if (z_flag_print_noinit_get(sh)) {
			z_shell_fprintf(sh, SHELL_WARNING, "%s", SHELL_MSG_BACKEND_NOT_ACTIVE);
...
static inline const struct shell_static_entry *selected_cmd_get(const struct shell *sh) {
...
static void tab_item_print(const struct shell *sh, const char *option,
	...
	columns =
		(sh->ctx->vt100_ctx.cons.terminal_wid - z_shell_strlen(tab)) / longest_option;
	rte_assert(columns != 0);
...
static void history_handle(const struct shell *sh, bool up) {
	...
	if (!z_shell_history_active(sh->history)) {
		if (up) {
			...
			if (cmd_len) {
				strcpy(sh->ctx->temp_buff, sh->ctx->cmd_buff);
			} else {
	...
	history_mode =
		z_shell_history_get(sh->history, up, (uint8_t *)sh->ctx->cmd_buff, &len);
...
static bool tab_prepare(const struct shell *sh, const struct shell_static_entry **cmd,
						const char ***argv, size_t *argc, size_t *complete_arg_idx,
	...
	memcpy(sh->ctx->temp_buff, sh->ctx->cmd_buff, sh->ctx->cmd_buff_pos);
	sh->ctx->temp_buff[sh->ctx->cmd_buff_pos] = '\0';

	/* Create argument list. */
	(void)z_shell_make_argv(argc, *argv, sh->ctx->temp_buff, CONFIG_SHELL_ARGC_MAX);
	...
	int space = (sh->ctx->cmd_buff_pos > 0)
					? isspace((int)sh->ctx->cmd_buff[sh->ctx->cmd_buff_pos - 1])
					: 0;
	...
	*cmd = z_shell_get_last_command(selected_cmd_get(sh), search_argc, *argv,
...
static inline bool is_completion_candidate(const char *candidate, const char *str,
										   size_t len) {
...
static void find_completion_candidates(const struct shell *sh,
									   ...
									   const char *incompl_cmd, size_t *first_idx,
									   size_t *cnt, uint16_t *longest) {
	...
	*cnt = 0;
	...
	while ((candidate = z_shell_cmd_get(cmd, idx, &dloc)) != NULL) {
		bool is_candidate;
		is_candidate =
			is_completion_candidate(candidate->syntax, incompl_cmd, incompl_cmd_len);
		if (is_candidate) {
...
static void autocomplete(const struct shell *sh, const struct shell_static_entry *cmd,
						 const char *arg, size_t subcmd_idx) {
	...
	if (cmd_len != arg_len) {
		z_shell_op_completion_insert(sh, match->syntax + arg_len, cmd_len - arg_len);
	}
...
static void tab_options_print(const struct shell *sh,
							  const struct shell_static_entry *cmd, const char *str,
							  size_t first, size_t cnt, uint16_t longest) {
	const struct shell_static_entry *match;
...
static void partial_autocomplete(const struct shell *sh,
	...
	uint16_t arg_len = z_shell_strlen(arg);
	uint16_t common = common_beginning_find(sh, cmd, &completion, first, cnt, arg_len);
	...
	if (common) {
		z_shell_op_completion_insert(sh, &completion[arg_len], common - arg_len);
	}
}
...
static int exec_cmd(const struct shell *sh, size_t argc, const char **argv,
	...
	if (sh->ctx->active_cmd.handler == NULL) {
		if ((help_entry != NULL) && IS_ENABLED(CONFIG_SHELL_HELP)) {
			...
			if (IS_ENABLED(CONFIG_SHELL_MSG_SPECIFY_SUBCOMMAND)) {
				z_shell_fprintf(sh, SHELL_ERROR, SHELL_MSG_SPECIFY_SUBCOMMAND);
			}
			return -ENOEXEC;
		}
	}
	...
	if (sh->ctx->active_cmd.args.mandatory) {
		...
		uint32_t opt = (opt8 == SHELL_OPT_ARG_CHECK_SKIP) ? UINT16_MAX : opt8;
...
static void active_cmd_prepare(const struct shell_static_entry *entry,
							   ...
							   struct shell_static_entry *help_entry, size_t *lvl,
							   size_t *handler_lvl, size_t *args_left) {
	if (entry->handler) {
		*handler_lvl = *lvl;
		*active_cmd = *entry;
...
static int execute(const struct shell *sh) {
	...
	if (IS_ENABLED(CONFIG_SHELL_HISTORY)) {
		z_shell_cmd_trim(sh);
		history_put(sh, (uint8_t *)sh->ctx->cmd_buff, sh->ctx->cmd_buff_len);
	}
	...
	if (parent != NULL) {
		...
		argv[1] = cmd_buf;
		argvp = &argv[1];
		active_cmd_prepare(parent, &sh->ctx->active_cmd, &help_entry, &cmd_lvl,
	...
	while ((argc != 1) && (cmd_lvl < CONFIG_SHELL_ARGC_MAX) && args_left > 0) {
		...
		if (IS_ENABLED(CONFIG_SHELL_WILDCARD) && (cmd_lvl > 0)) {
			...
			status = z_shell_wildcard_process(sh, entry, argvp[0]);
		...
		args_left--;
		if (entry) {
			if (wildcard_check_report(sh, wildcard_found, entry) == false) {
				return -ENOEXEC;
			}

			active_cmd_prepare(entry, &sh->ctx->active_cmd, &help_entry, &cmd_lvl,
							   &cmd_with_handler_lvl, &args_left);
			parent = entry;
		} else {
			if (IS_ENABLED(CONFIG_SHELL_MSG_CMD_NOT_FOUND) && cmd_lvl == 0 &&
				(!z_shell_in_select_mode(sh) || sh->ctx->selected_cmd->handler == NULL)) {
				z_shell_fprintf(sh, SHELL_ERROR, "%s%s\n", argv[0],
	...
	if (IS_ENABLED(CONFIG_SHELL_WILDCARD) && wildcard_found) {
		 ...
		 * be called again.
		 */
		(void)z_shell_make_argv(&cmd_lvl, &argv[selected_cmd_get(sh) ? 1 : 0],
								sh->ctx->cmd_buff, CONFIG_SHELL_ARGC_MAX);
	...
	return exec_cmd(sh, cmd_lvl - cmd_with_handler_lvl, &argv[cmd_with_handler_lvl],
					&help_entry);
}
...
static void tab_handle(const struct shell *sh) {
	...
	bool tab_possible = tab_prepare(sh, &cmd, &argv, &argc, &arg_idx, &d_entry);

	if (tab_possible == false) {
		return;
	}

	find_completion_candidates(sh, cmd, argv[arg_idx], &first, &cnt, &longest);
...
static void alt_metakeys_handle(const struct shell *sh, char data) {
	...
	if (data == SHELL_VT100_ASCII_ALT_B) {
		...
		z_shell_op_cursor_word_move(sh, 1);
	} else if (data == SHELL_VT100_ASCII_ALT_R && IS_ENABLED(CONFIG_SHELL_CMDS_SELECT)) {
...
static void state_collect(const struct shell *sh) {
	...
	while (true) {
		...
		if (bypass) {
			...
			(void)sh->iface->api->read(sh->iface, buf, sizeof(buf), &count);
			if (count) {
				z_flag_cmd_ctx_set(sh, true);
				bypass(sh, buf, count);
				z_flag_cmd_ctx_set(sh, false);
		...
		case SHELL_RECEIVE_DEFAULT:
			...
			case '\t': /* TAB */
				if (z_flag_echo_get(sh) && IS_ENABLED(CONFIG_SHELL_TAB)) {
...
static void transport_evt_handler(enum shell_transport_evt evt_type, void *ctx) {
	...
	if (evt_type == SHELL_TRANSPORT_EVT_RX_RDY)
		events = SHELL_SIGNAL_RXRDY;
	else
		events = SHELL_SIGNAL_TXDONE;
	tx_event_flags_set(&sh->ctx->event_group, events, TX_OR);
}
...
static void shell_log_process(const struct shell *sh) {
	...
	do {
		 ...
		 */
		if (sh->ctx->cmd_buff_len) {
			tx_thread_sleep(TX_MSEC(15));
		}

		result = tx_event_flags_get(&sh->ctx->event_group, SHELL_SIGNAL_RXRDY, TX_AND,
									&signaled, TX_NO_WAIT);
		// k_poll_signal_check(signal, &signaled, &result);
	} while (processed && !result);
}

static int instance_init(const struct shell *sh, const void *transport_config,
						 struct shell_backend_config_flags cfg_flags) {
	rte_assert((sh->shell_flag == SHELL_FLAG_CRLF_DEFAULT) ||
	...
	tx_mutex_create(&sh->ctx->wr_mtx, "shell", TX_INHERIT);
	tx_event_flags_create(&sh->ctx->event_group, "shell");
	...
	int ret = sh->iface->api->init(sh->iface, transport_config, transport_evt_handler,
								   (void *)sh);
	if (ret == 0) {
...
static int instance_uninit(const struct shell *sh) {
	...
	tx_mutex_delete(&sh->ctx->wr_mtx);
	tx_event_flags_delete(&sh->ctx->event_group);
...
void shell_thread(void *arg) {
	struct shell_threadparam *param = arg;
	ULONG req_events = SHELL_SIGNAL_RXRDY | SHELL_SIGNAL_LOG_MSG | SHELL_SIGNAL_KILL;
	struct shell *sh = param->shell;
	int err;

	z_flag_handle_log_set(sh, param->log_backend);
	sh->ctx->log_level = POINTER_TO_UINT(param->log_level);
	...
	while (true) {
		ULONG shell_events = 0;

		/* waiting for all signals except SHELL_SIGNAL_TXDONE */
		err = tx_event_flags_get(&sh->ctx->event_group, req_events, TX_OR_CLEAR,
								 &shell_events, TX_WAIT_FOREVER);
		if (err != TX_SUCCESS) {
			if (tx_mutex_get(&sh->ctx->wr_mtx, TX_MSEC(SHELL_TX_MTX_TIMEOUT_MS)) != 0) {
				return;
			}
			z_shell_fprintf(sh, SHELL_ERROR, "Shell thread error: %d", err);
			tx_mutex_put(&sh->ctx->wr_mtx);
			return;
		}
		...
		if (shell_events & SHELL_SIGNAL_KILL)
			kill_handler(sh);

		if (shell_events & SHELL_SIGNAL_RXRDY)
			shell_process(sh);

		if (IS_ENABLED(CONFIG_SHELL_LOG_BACKEND)) {
			if (shell_events & SHELL_SIGNAL_LOG_MSG)
...
void shell_uninit(const struct shell *sh, shell_uninit_cb_t cb) {
	...
	if (IS_ENABLED(CONFIG_MULTITHREADING)) {
		sh->ctx->uninit_cb = cb;
		/* signal kill message */
		tx_event_flags_set(&sh->ctx->event_group, SHELL_SIGNAL_KILL, TX_OR);
		return;
	}
...
void shell_vfprintf(const struct shell *sh, enum shell_vt100_color color, const char *fmt,
					va_list args) {
	rte_assert(sh);
	// rte_assert(!k_is_in_isr());
	rte_assert(sh->ctx);
	rte_assert(z_flag_cmd_ctx_get(sh) || (_tx_thread_identify() != sh->ctx->tid));
...
void shell_hexdump_line(const struct shell *sh, unsigned int offset, const uint8_t *data,
						size_t len) {
	rte_assert(sh);
	...
	for (i = 0; i < SHELL_HEXDUMP_BYTES_IN_LINE; i++) {
		...
		if (i < len) {
			shell_fprintf_normal(sh, "%02x ", data[i] & 0xFF);
		} else {
			shell_fprintf_normal(sh, "   ");
		}
	}
	...
	for (i = 0; i < SHELL_HEXDUMP_BYTES_IN_LINE; i++) {
		...
		if (i < len) {
			...
			shell_fprintf_normal(sh, "%c", isprint((int)c) != 0 ? c : '.');
		} else {
			shell_fprintf_normal(sh, " ");
		}
	}
...
int shell_execute_cmd(const struct shell *sh, const char *cmd) {
	...
	rte_assert(!z_flag_cmd_ctx_get(sh));
