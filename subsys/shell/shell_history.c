...
#include <string.h>
#include <subsys/shell/shell_history.h>
...
bool z_shell_history_get(struct shell_history *history, bool up, uint8_t *dst,
						 uint16_t *len) {
	struct shell_history_item *h_item; /* history item */
	struct rte_list *l_item;		   /* list item */

	if (rte_list_empty(&history->list)) {
		*len = 0U;
		return false;
	}
...
static void add_to_head(struct shell_history *history, struct shell_history_item *item,
						uint8_t *src, size_t len, uint16_t padding) {
	item->len = len;
	item->padding = padding;
	memcpy(item->data, src, len);
	rte_list_add(&item->dnode, &history->list);
}
...
static bool remove_from_tail(struct shell_history *history) {
	...
	l_item = history->list.prev; // sys_dlist_peek_tail(&history->list);
	rte_list_del(l_item);

	h_item = CONTAINER_OF(l_item, struct shell_history_item, dnode);

	total_len = offsetof(struct shell_history_item, data) + h_item->len + h_item->padding;
...
void z_shell_history_put(struct shell_history *history, uint8_t *line, size_t len) {
	...
	if (!rte_list_empty(&history->list)) {
		l_item = history->list.next; // sys_dlist_peek_head(&history->list);
	...
	do {
		...
		claim_len = ring_buf_put_claim(history->ring_buf, (uint8_t **)&h_item, total_len);
		/* second allocation may succeed if we were at the end of the
		 * buffer.
		 */
		if (claim_len < total_len) {
			claim2_len =
				ring_buf_put_claim(history->ring_buf, (uint8_t **)&h_item, total_len);
