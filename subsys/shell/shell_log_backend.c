...
bool z_shell_log_backend_process(const struct shell_log_backend *backend) {
	const struct shell *sh = (const struct shell *)backend->backend->cb->ctx;
	uint32_t dropped;
	bool colors = IS_ENABLED(CONFIG_SHELL_VT100_COLORS) && z_flag_use_colors_get(sh);
...
static void panic(const struct log_backend *const backend) {
	...
	if (err == 0) {
		sh->log_backend->control_block->state = SHELL_LOG_BACKEND_PANIC;
		z_flag_sync_mode_set(sh, true);
		...
		z_shell_op_cursor_horiz_move(sh, -sh->ctx->vt100_ctx.cons.cur_x);
...
static void process_log_msg(const struct shell *sh, const struct log_output *log_output,
							union log_msg_generic *msg, bool locked, bool colors) {
	unsigned int key = 0;
	uint32_t flags =
		LOG_OUTPUT_FLAG_LEVEL | LOG_OUTPUT_FLAG_TIMESTAMP | LOG_OUTPUT_FLAG_THREAD;
