 ...
 */
#include <base/sys/iterable_sections.h>
...
#define SHELL_HELP_STATISTICS_RESET "Reset shell statistics for the Logger module."
...
static int cursor_position_get(const struct shell *sh, uint16_t *x, uint16_t *y) {
	...
	*y = 0U;
	...
	for (uint16_t i = 0; i < 1000; i++) {
		do {
			(void)sh->iface->api->read(sh->iface, &c, sizeof(c), &cnt);
			if (cnt == 0) {
			...
			if ((c != SHELL_VT100_ASCII_ESC) &&
				(sh->ctx->temp_buff[0] != SHELL_VT100_ASCII_ESC)) {
				continue;
			}
			...
			if (c == 'R') { /* End of response from the terminal. */
				...
				while (sh->ctx->temp_buff[buff_idx] != ';') {
					*y = *y * 10U + (sh->ctx->temp_buff[buff_idx++] - '0');
				...
				while (sh->ctx->temp_buff[buff_idx] != '\0') {
					*x = *x * 10U + (sh->ctx->temp_buff[buff_idx++] - '0');
...
static int cmd_bacskpace_mode_delete(const struct shell *sh, size_t argc, char **argv) {
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_echo(const struct shell *sh, size_t argc, char **argv) {
	if (argc == 2) {
		shell_error(sh, "%s:%s%s", argv[0], SHELL_MSG_UNKNOWN_PARAMETER, argv[1]);
		return -EINVAL;
	}

	shell_print(sh, "Echo status: %s", z_flag_echo_get(sh) ? "on" : "off");
...
static int cmd_history(const struct shell *sh, size_t argc, char **argv) {
	...
	while (1) {
		z_shell_history_get(sh->history, true, (uint8_t *)sh->ctx->temp_buff, &len);

		if (len) {
			shell_print(sh, "[%3d] %s", (int)i++, sh->ctx->temp_buff);
...
static int cmd_shell_stats_show(const struct shell *sh, size_t argc, char **argv) {
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_shell_stats_reset(const struct shell *sh, size_t argc, char **argv) {
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_resize_default(const struct shell *sh, size_t argc, char **argv) {
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_select(const struct shell *sh, size_t argc, char **argv) {
	...
	argc--;
	argv = argv + 1;
	candidate = z_shell_get_last_command(sh->ctx->selected_cmd, argc, (const char **)argv,
										 &matching_argc, &entry, true);

	if ((candidate != NULL) && !no_args(candidate) && (argc == matching_argc)) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(m_sub_colors,
							   SHELL_COND_CMD_ARG(CONFIG_SHELL_VT100_COMMANDS, off, NULL,
												  SHELL_HELP_COLORS_OFF, cmd_colors_off,
												  1, 0),
							   SHELL_COND_CMD_ARG(CONFIG_SHELL_VT100_COMMANDS, on, NULL,
												  SHELL_HELP_COLORS_ON, cmd_colors_on, 1,
												  0),
							   SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(m_sub_vt100,
							   SHELL_COND_CMD_ARG(CONFIG_SHELL_VT100_COMMANDS, off, NULL,
												  SHELL_HELP_VT100_OFF, cmd_vt100_off, 1,
												  0),
							   SHELL_COND_CMD_ARG(CONFIG_SHELL_VT100_COMMANDS, on, NULL,
												  SHELL_HELP_VT100_ON, cmd_vt100_on, 1,
												  0),
							   SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(
	m_sub_prompt, SHELL_CMD_ARG(off, NULL, SHELL_HELP_PROMPT_OFF, cmd_prompt_off, 1, 0),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(on, NULL, SHELL_HELP_ECHO_ON, cmd_echo_on, 1, 0), SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	m_sub_shell, SHELL_CMD_ARG(backends, NULL, SHELL_HELP_BACKENDS, cmd_backends, 1, 0),
	SHELL_CMD(backspace_mode, &m_sub_backspace_mode, SHELL_HELP_BACKSPACE_MODE, NULL),
	SHELL_COND_CMD(CONFIG_SHELL_VT100_COMMANDS, colors, &m_sub_colors, SHELL_HELP_COLORS,
				   NULL),
	SHELL_COND_CMD(CONFIG_SHELL_VT100_COMMANDS, vt100, &m_sub_vt100, SHELL_HELP_VT100,
				   NULL),
	SHELL_CMD(prompt, &m_sub_prompt, SHELL_HELP_PROMPT, NULL),
	SHELL_CMD_ARG(echo, &m_sub_echo, SHELL_HELP_ECHO, cmd_echo, 1, 1),
	SHELL_COND_CMD(CONFIG_SHELL_STATS, stats, &m_sub_shell_stats, SHELL_HELP_STATISTICS,
				   NULL),
	SHELL_SUBCMD_SET_END);
...
SHELL_COND_CMD_REGISTER(CONFIG_SHELL_VT100_COMMANDS, rem, NULL, SHELL_HELP_COMMENT,
						cmd_comment);
SHELL_COND_CMD_ARG_REGISTER(CONFIG_SHELL_VT100_COMMANDS, clear, NULL, SHELL_HELP_CLEAR,
							cmd_clear, 1, 0);
SHELL_CMD_REGISTER(shell, &m_sub_shell, SHELL_HELP_SHELL, NULL);
SHELL_COND_CMD_ARG_REGISTER(CONFIG_SHELL_HISTORY, history, NULL, SHELL_HELP_HISTORY,
							cmd_history, 1, 0);
SHELL_COND_CMD_ARG_REGISTER(CONFIG_SHELL_CMDS_RESIZE, resize, &m_sub_resize,
							SHELL_HELP_RESIZE, cmd_resize, 1, 1);
SHELL_COND_CMD_ARG_REGISTER(CONFIG_SHELL_CMDS_SELECT, select, NULL, SHELL_HELP_SELECT,
							cmd_select, 2, SHELL_OPT_ARG_CHECK_SKIP);
