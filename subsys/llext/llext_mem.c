...
#define pr_fmt(fmt) "[llext_mem]: "fmt "\n"
#include <errno.h>
#include <string.h>
...
BYTE_HEAP_DEFINE(llext_heap, CONFIG_LLEXT_HEAP_SIZE * 1024, );
...
static void llext_init_mem_part(struct llext *ext, enum llext_mem mem_idx,
	...
	pr_dbg("region %d: start 0x%zx, size %zd", mem_idx, (size_t)start, len);
}
...
static int llext_copy_section(struct llext_loader *ldr, struct llext *ext,
	...
	llext_init_mem_part(ext, mem_idx, (uintptr_t)ext->mem[mem_idx], sect_alloc);
...
int llext_copy_regions(struct llext_loader *ldr, struct llext *ext,
	...
	if (IS_ENABLED(CONFIG_LLEXT_LOG_LEVEL_DBG)) {
		pr_dbg("gdb add-symbol-file flags:");
...
void llext_adjust_mmu_permissions(struct llext *ext) {
	...
	for (enum llext_mem mem_idx = 0; mem_idx < LLEXT_MEM_PARTITIONS; mem_idx++) {
		...
		switch (mem_idx) {
		case LLEXT_MEM_TEXT:
			cache_instr_invd_range(addr, size);
		...
		cache_data_flush_range(addr, size);
...
void llext_free_regions(struct llext *ext) {
	for (int i = 0; i < LLEXT_MEM_COUNT; i++) {
		...
		if (ext->mem_on_heap[i]) {
			pr_dbg("freeing memory region %d", i);
