...
#define pr_fmt(fmt) "[llext_link]: "fmt "\n"
...
void llext_dependency_remove_all(struct llext *ext) {
	...
	for (i = 0; i < ARRAY_SIZE(ext->dependency) && ext->dependency[i]; i++) {
		...
		rte_assert_msg(ext->dependency[i]->use_count, "LLEXT dependency use-count underrun!");
...
static void llext_link_plt(struct llext_loader *ldr, struct llext *ext, elf_shdr_t *shdr,
	...
	pr_dbg("Found %p in PLT %"PRIu32" size %"PRIu32" cnt %u text %p",
	...
	for (unsigned int i = 0; i < sh_cnt; i++) {
		...
		if (ret != 0) {
			pr_err("PLT: failed to read RELA #%u, trying to continue", i);
			continue;
		}
		...
		if (ret != 0) {
			pr_err("PLT: failed to read symbol table #%u RELA #%u, trying to continue",
					j, i);
			continue;
		}
		 ...
		 */
		uint8_t *rel_addr =
			(uint8_t *)ext->mem[LLEXT_MEM_TEXT] - ldr->sects[LLEXT_MEM_TEXT].sh_offset;
		...
		if (tgt) {
			...
			if (offset < 0) {
				pr_err("Offset %#zx not found in ELF, trying to continue",
		...
		case STB_GLOBAL:
			...
			if (!link_addr) {
				pr_warn("PLT: cannot find idx %u name %s", j, name);
				continue;
			}
		...
		pr_dbg("symbol %s relocation @%p r-offset %#zx .text offset %#zx stb %"PRIu32, name,
				(void *)rel_addr, (size_t)rela.r_offset,
...
int llext_link(struct llext_loader *ldr, struct llext *ext,
	...
	for (i = 0; i < (int)ext->sect_cnt; ++i) {
		...
		case SHT_REL:
			if (shdr->sh_entsize != sizeof(elf_rel_t)) {
				pr_err("Invalid entry size %zd for SHT_REL section %d",
		...
		case SHT_RELA:
			if (IS_ENABLED(CONFIG_ARM)) {
				pr_err("Found unsupported SHT_RELA section %d", i);
				return -ENOTSUP;
			}
			if (shdr->sh_entsize != sizeof(elf_rela_t)) {
				pr_err("Invalid entry size %zd for SHT_RELA section %d",
						(size_t)shdr->sh_entsize, i);
				return -ENOEXEC;
			}
			break;
		...
		if (shdr->sh_info >= ext->sect_cnt || shdr->sh_size % shdr->sh_entsize != 0) {
			pr_err("Sanity checks failed for section %d "
		...
		pr_dbg("relocation section %s (%d) acting on section %"PRId32" has %"PRIu32" relocations",
				name, i, shdr->sh_info, rel_cnt);

		enum llext_mem mem_idx = ldr->sect_map[shdr->sh_info].mem_idx;

		if (mem_idx == LLEXT_MEM_COUNT) {
			pr_err("Section %"PRId32" not loaded in any memory region", shdr->sh_info);
			return -ENOEXEC;
		}

		sect_base = (uintptr_t)llext_loaded_sect_ptr(ldr, ext, shdr->sh_info);

		for (int j = 0; j < (int)rel_cnt; j++) {
			...
			pr_dbg("relocation %d:%d info 0x%zx (type %zd, sym %zd) offset %zd "
			...
			if (ELF_R_SYM(rel.r_info) == 0) {
				...
				link_addr =
					(uintptr_t)llext_find_sym(NULL, SYM_NAME_OR_SLID(name, sym.st_value));
				...
				if (link_addr == 0) {
					pr_err("Undefined symbol with no entry in "
							"symbol table %s, offset %"PRIu32", link section %"PRId32,
							name, rel.r_offset, shdr->sh_link);
					return -ENODATA;
				}

				pr_info("found symbol %s at %p", name, (void *)link_addr);
				...
				pr_err("rela section %d, entry %d: cannot apply relocation: "
						"target symbol has unexpected section index %d (0x%x)",
						i, j, sym.st_shndx, sym.st_shndx);
				return -ENOEXEC;
			}

			pr_info("writing relocation symbol %s type %"PRIu32" sym %"PRIu32" at addr %p "
					"addr %p",
					name, ELF_R_TYPE(rel.r_info), ELF_R_SYM(rel.r_info),
					(void *)op_loc, (void *)link_addr);
	...
	for (i = 0; i < LLEXT_MEM_COUNT; ++i) {
		if (ext->mem[i]) {
			cache_data_flush_range(ext->mem[i], ext->mem_size[i]);
			cache_instr_invd_range(ext->mem[i], ext->mem_size[i]);
		}
	}
	...
	if (ldr_parm->section_detached) {
		for (i = 0; i < ext->sect_cnt; ++i) {
			...
			if (ldr_parm->section_detached(shdr)) {
				...
				cache_data_flush_range(base, shdr->sh_size);
				cache_instr_invd_range(base, shdr->sh_size);
			}
		}
	}
#endif
