...
static STAILQ_HEAD(, llext) _llext_list = STAILQ_HEAD_INITIALIZER(_llext_list);
static TX_MUTEX llext_lock;
...
int llext_section_shndx(const struct llext_loader *ldr, const struct llext *ext,
	...
	for (i = 1; i < ext->sect_cnt; i++) {
		const char *name =
			llext_string(ldr, ext, LLEXT_MEM_SHSTRTAB, ext->sect_hdrs[i].sh_name);
...
int llext_get_section_header(struct llext_loader *ldr, struct llext *ext,
...
ssize_t llext_find_section(struct llext_loader *ldr, const char *search_name) {
	...
	for (i = 0, pos = ldr->hdr.e_shoff; i < ldr->hdr.e_shnum;
		...
		const char *name =
			llext_peek(ldr, ldr->sects[LLEXT_MEM_SHSTRTAB].sh_offset + shdr->sh_name);
...
struct llext *llext_by_name(const char *name) {
	...
	guard(os_mutex)(&llext_lock);
	STAILQ_FOREACH(ext, &_llext_list, _llext_list) {
...
int llext_iterate(int (*fn)(struct llext *ext, void *arg), void *arg) {
	...
	scoped_guard(os_mutex, &llext_lock) {
		STAILQ_FOREACH(ext, &_llext_list, _llext_list) {
			ret = fn(ext, arg);
			if (ret) {
				break;
...
int llext_load(struct llext_loader *ldr, const char *name, struct llext **ext,
	...
	*ext = llext_alloc(sizeof(struct llext));
	if (*ext == NULL) {
		pr_err("Not enough memory for extension metadata");
		ret = -ENOMEM;
		goto out;
	}
	...
	STAILQ_INSERT_TAIL(&_llext_list, *ext, _llext_list);
	pr_info("Loaded extension %s", (*ext)->name);
...
int llext_unload(struct llext **ext) {
	rte_assert_msg(*ext, "Expected non-null extension");
	...
	scoped_guard(os_mutex, &llext_lock) {
		rte_assert_msg(tmp->use_count, "A valid LLEXT cannot have a zero use-count!");
		...
		STAILQ_REMOVE(&_llext_list, tmp, llext, _llext_list);
...
static int call_fn_table(struct llext *ext, bool is_init) {
	...
	if (ret < 0) {
		pr_err("Failed to get table size: %d", (int)ret);
		return ret;
	}
	...
	if (ret < 0) {
		pr_err("Failed to get function table: %d", (int)ret);
		return ret;
	}

	for (int i = 0; i < fn_count; i++) {
		pr_dbg("calling %s function %p()", is_init ? "bringup" : "teardown",
				(void *)fn_table[i]);
		fn_table[i]();
	}
...
void llext_bootstrap(struct llext *ext, llext_entry_fn_t entry_fn, void *user_data) {
	...
	if (ret < 0) {
		pr_err("Failed to call init functions: %d", ret);
		return;
	}

	/* Start extension main function */
	pr_dbg("calling entry function %p(%p)", (void *)entry_fn, user_data);
	entry_fn(user_data);

	/* Call de-initialization functions */
	ret = llext_teardown(ext);
	if (ret < 0) {
		pr_err("Failed to call de-init functions: %d", ret);
		return;
	}
}

static int llext_init(void) {
	return (int)tx_mutex_create(&llext_lock, (CHAR *)"llext", TX_INHERIT);
}

SYSINIT(llext_init, SI_EARLY_LEVEL, 50);
