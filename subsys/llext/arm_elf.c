...
static inline int prel31_decode(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
								const char *sym_name, int32_t *offset) {
	int ret;
...
static int prel31_handler(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
						  const char *sym_name) {
	int ret;
	int32_t offset;
...
static inline int jumps_decode(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
							   const char *sym_name, int32_t *offset) {
	int ret;
...
static int jumps_handler(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
						 const char *sym_name) {
	int ret;
	int32_t offset;
...
static void movs_handler(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
						 const char *sym_name) {
	int32_t offset;
	uint32_t tmp;
...
static inline int thm_jumps_decode(elf_word reloc_type, uint32_t loc,
								   ...
								   int32_t *offset, uint32_t *upper, uint32_t *lower) {
	int ret;
...
static inline void thm_jumps_reloc(uint32_t loc, int32_t *offset, uint32_t *upper,
								   uint32_t *lower) {
	uint32_t j_one, j_two, sign;
...
static int thm_jumps_handler(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
							 const char *sym_name) {
	int ret;
	int32_t offset;
	uint32_t upper, lower;

	ret = thm_jumps_decode(reloc_type, loc, sym_base_addr, sym_name, &offset, &upper,
						   &lower);
	if (!ret) {
...
static void thm_movs_handler(elf_word reloc_type, uint32_t loc, uint32_t sym_base_addr,
							 const char *sym_name) {
	int32_t offset;
	uint32_t upper, lower;
	...
	offset = ((upper & MASK_THM_MOV_IMM4) << SHIFT_THM_MOV_IMM4) |
			 ...
			 ((lower & MASK_THM_MOV_IMM3) >> SHIFT_THM_MOV_IMM3) |
			 (lower & MASK_THM_MOV_IMM8);
