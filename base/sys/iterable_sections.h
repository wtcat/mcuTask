...
#ifndef BASE_SYS_ITERABLE_SECTIONS_H_
#define BASE_SYS_ITERABLE_SECTIONS_H_
...
#define TYPE_SECTION_DEFINE(type, secname) LINKER_ROSET(type, secname)
...
#define TYPE_SECTION_ITERABLE(type, varname, secname, section_postfix) \
    LINKER_SET_ALIGN(type) type const varname \
    __rte_section(ROSET_SORTED_SECTION "." #secname ".content.0." _XSTRING(section_postfix)) __rte_used\
 ...
 *
 */
#define TYPE_SECTION_START(secname) LINKER_SET_BEGIN(secname)
 ...
 * would be TYPE_SECTION_START(foobar)
 */
#define TYPE_SECTION_END(secname) LINKER_SET_END(secname)
...
#define STRUCT_SECTION_ITERABLE_ARRAY_ALTERNATE(secname, struct_type, varname, size)  \
	TYPE_SECTION_ITERABLE(struct struct_type, varname[size], secname, varname)
...
#endif /* BASE_SYS_ITERABLE_SECTIONS_H_ */
